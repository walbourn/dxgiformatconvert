...
//-----------------------------------------------------------------------------
// B8G8R8A8_UNORM_SRGB Test Shader
//-----------------------------------------------------------------------------
[numthreads(1,1,1)]
void main()
{
    ...
    for(uint y = 0; y < height; y++)
    {
        for(uint x = 0; x < width; x++)
        {
            ...
            values.HLSLResult = asuint(D3DX_B8G8R8A8_UNORM_SRGB_to_FLOAT4(values.testVal));
            values.outVal = D3DX_FLOAT4_to_B8G8R8A8_UNORM_SRGB(asfloat(values.RefResult));
    ...
    if( uUINT[uint2(0,0)] == 0 )
    {
        ...
        values.testVal = D3DX_FLOAT4_to_B8G8R8A8_UNORM_SRGB(asfloat(values.RefResult));
        values.outVal = D3DX_FLOAT4_to_B8G8R8A8_UNORM_SRGB(asfloat(values.HLSLResult));
