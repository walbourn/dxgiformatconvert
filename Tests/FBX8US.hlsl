...
//-----------------------------------------------------------------------------
// B8G8R8X8_UNORM_SRGB Test Shader
//-----------------------------------------------------------------------------
[numthreads(1,1,1)]
void main()
{
    for(uint y = 0; y < uUINT.Length.y; y++)
    {
        for(uint x = 0; x < uUINT.Length.x; x++)
        {
            ...
            values.HLSLResult = uint4(asuint(D3DX_B8G8R8X8_UNORM_SRGB_to_FLOAT3(values.testVal)),asuint(1.0f));
            values.outVal = D3DX_FLOAT3_to_B8G8R8X8_UNORM_SRGB(asfloat(values.RefResult.xyz));
    ...
    if( uUINT[uint2(0,0)] == 0 )
    {
        ...
        values.testVal = D3DX_FLOAT3_to_B8G8R8X8_UNORM_SRGB(asfloat(values.RefResult.xyz));
        values.outVal = D3DX_FLOAT3_to_B8G8R8X8_UNORM_SRGB(asfloat(values.HLSLResult.xyz));
